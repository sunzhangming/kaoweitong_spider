        ...
        if "0; URL=/cn/MyHome/?" in text:
            ...
            if len(data) != 0:
                if data[0] == self.test_date:
                    ...
                    result = {"test_date":"%s"%deal_with_time(self.test_date),"reading":int(score_res[2]), "listening":int(score_res[3]), "speaking":int(score_res[4]), "writing":int(score_res[5])}
                    ...
                    # res = '%s出成绩了！！{"reading":%s, "listening":score_res[3], "speaking":score_res[4], "writing":score_res[5]'%(self.openid, score_res[2])
                    return result
                return "没有出分"
            return "没有出分"
...
def main():
    ...
    test_date = "2017年3月25日"
    openid = "testtest"
    redis_ = Redis()
    mysql = Mysql()
    all_task_list1 = [] # ["2018-01-01"]
    all_task_list2 = [] # [{"isFished":0}]
    all_task_list3 = [] # ["2018-01-01"]
    all_no_chengji = mysql.query_all_no_chengji()
    for no_chengji in all_no_chengji:
        if no_chengji[8] not in all_task_list1:
            all_task_list1.append(no_chengji[8])
            all_task_list2.append(no_chengji)
    for task_ in redis_.smembers("all_task"):
            all_task_list3.append(task_.decode().split(",")[2])
    for task in all_task_list1:
        if task not in all_task_list3:
            index = all_task_list1.index(task)
            redis_.sadd("all_task", "%s,%s,%s,%s"%(all_task_list2[index][2],all_task_list2[index][3],all_task_list2[index][8],all_task_list2[index][1]))

    task_list = []
    all_task = redis_.smembers("all_task")
    print(all_task)
    for task in all_task:
        line_list = task.decode().split(",")
        mysql = Mysql()
        res = main2(mysql, line_list[3] ,line_list[0], line_list[1], line_list[2])
        mysql.close()
        end_time = datetime.now()
        print((end_time - start_time).seconds)
        if res == "ok":
            redis_.sadd("task_son",line_list[2])


def query(openid, neea, neea_pwd, test_date):
    neea = neea
    neea_pwd = neea_pwd
    openid = openid
    test_date = test_date
    test_date = time2china_time(test_date)
    main2(openid, neea, neea_pwd, test_date)
...
if __name__ == '__main__':
    while True:
        ...
        main()
        redis2 = Redis()
        task_son_list = redis2.smembers("task_son")
        if task_son_list:
            for task_son in task_son_list:
                os.system("nohup python3 chengji_son.py &")
